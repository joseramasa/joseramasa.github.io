---
layout: course
title: PQC & QRNG Seminars — Università di Bari
description: Invited seminars on Post-Quantum Cryptography and Quantum Random Number Generation at the Master of Quantum Science and Technology.
instructor: José R. Martínez Saavedra
year: 2025
term: Summer
location: Università degli Studi di Bari Aldo Moro, Italy
time: 6 hours (2 sessions)
course_id: bari-pqc-qrng
schedule:
  - week: 1
    date: July 7, 2025
    topic: Post-Quantum Cryptography
    description: Overview of post-quantum cryptographic algorithms, lattice-based schemes, and their integration with quantum-safe infrastructure.
    materials:
      - name: Slides (PDF)
        url: /assets/teaching/bari-pqc-2025.pdf

  - week: 2
    date: July 8, 2025
    topic: Quantum Random Number Generation
    description: Fundamentals of quantum randomness, QRNG device architectures, certification, and deployment in real-world applications.
    materials:
      - name: Slides (PDF)
        url: /assets/teaching/bari-qrng-2025.pdf
---

## Course Overview

Invited seminars at the [Master of Quantum Science and Technology](https://www.masterquantum.it/it/home-2/) at Università degli Studi di Bari Aldo Moro (Italy). Two sessions of 3 hours each covering post-quantum cryptography and quantum random number generation.

**Institution:** Università degli Studi di Bari Aldo Moro
